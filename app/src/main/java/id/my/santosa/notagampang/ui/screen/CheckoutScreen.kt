...
fun CheckoutScreen(
  ...
  val totalToPay =
    if (isReadOnly) {
      (uiState.unpaidItems + uiState.paidItems).sumOf { it.priceAtOrder * it.quantity }
    } else {
  ...
  Box(modifier = Modifier.fillMaxSize()) {
    if (uiState.isLoading) {
      ...
      LazyColumn(
        ...
        val itemsToDisplay =
          if (isReadOnly) {
            uiState.unpaidItems + uiState.paidItems
          } else {
            uiState.unpaidItems
          }

        items(itemsToDisplay, key = { it.id }) { item ->
          val isSelected = isReadOnly || uiState.selectedItemIds.contains(item.id)
          Card(
            modifier =
              Modifier.fillMaxWidth()
                .padding(
          ...
          item {
            ...
            Column(
              ...
              OutlinedTextField(
                ...
                trailingIcon = {
                  TextButton(
                    onClick = { cashReceivedStr = totalToPay.toString() },
                  ) {
                    Text(
                      "UANG PAS",
                      style = MaterialTheme.typography.labelSmall,
              ...
              if (isKasbon && totalToPay > 0) {
                Card(
                  ...
                  Column(
                    ...
                    Text(
                      "Sisa Pembayaran: ${currencyFormat.format(
      ...
      Surface(
        ...
        Column {
          ...
          Row(
            ...
            val itemsCount =
              if (isReadOnly) {
                (uiState.unpaidItems + uiState.paidItems).count()
              } else {
                uiState.unpaidItems.count { uiState.selectedItemIds.contains(it.id) }
              }
            Column {
              Text(
                "Total",
            ...
            if (isReadOnly || !isKasbon) {
              Button(
                ...
                enabled = isReadOnly || uiState.selectedItemIds.isNotEmpty(),
                shape = MaterialTheme.shapes.medium,
                colors =
